...
X(구 Twitter)를 모티브로 한 소셜 네트워킹 서비스입니다. Next.js와 Nestjs를 기반으로 개발되었으며, 실시간 상호작용과 최적화된 사용자 경험을 제공합니다. 모바일 환경을 고려한 반응형 UI를 구현하여 스마트폰, 태블릿, 데스크톱 등 다양한 디바이스에서 최적화된 사용자 경험을 제공합니다.
...
- 좋아요, 게시물 작성 등의 사용자 액션에 대해 즉각적인 UI 피드백 제공
- 서버 응답을 기다리지 않고 UI를 먼저 업데이트하여 더 나은 사용자 경험 제공
- 실패 시 자동으로 이전 상태로 롤백되어 데이터 일관성 유지
- React Query의 `useMutation` 훅을 활용한 효율적인 상태 관리

### 7. 검색 기능 (현재는 api만 구현되어있음)
